...
import deleteTriggerAction from '@salesforce/apex/TriggerActionService.deleteTriggerAction';
...
// Group by context, sorted in CONTEXT_LABELS order, then by Order__c within each group
const sections = [];
...
handleDelete() {
if (!this.selectedAction) {
this.showWarning('Please select an action to delete');
return;
}
...
this.isLoading = true;
try {
await deleteTriggerAction({ devName: this.selectedAction.DeveloperName });
this.selectedAction = null;
this.showSuccess('Trigger Action deleted successfully.');
await this.refreshList();
} catch (error) {
this.showError('Delete Error', error.body?.message || error.message);
} finally {
this.isLoading = false;
}
}
...
return this.selectedAction?.DeveloperName || '';
}
